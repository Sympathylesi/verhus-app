...
const EMPTY = { name: '', districtId: '', population0_11m: '', population12_23m: '', population24_59m: '' };

export default function HealthAreaForm({ open, onClose, onSubmit, initial, districts, lang }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!form.name.trim() || !form.districtId) return;
    onSubmit({
      name: form.name.trim(),
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-2">
          ...
          <div className="space-y-1.5">
            ...
            <Select value={form.districtId} onValueChange={v => setForm(f => ({ ...f, districtId: v }))}>
              <SelectTrigger>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="space-y-1.5">
              ...
              <Input
                type="number"
                min={0}
                value={form.population0_11m}
                onChange={set('population0_11m')}
                placeholder="0"
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                type="number"
                min={0}
                value={form.population12_23m}
                onChange={set('population12_23m')}
                placeholder="0"
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                type="number"
                min={0}
                value={form.population24_59m}
                onChange={set('population24_59m')}
                placeholder="0"
              />
            </div>
          </div>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={!form.name.trim() || !form.districtId}>{t('Save', 'Enregistrer')}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>

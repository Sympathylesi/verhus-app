...
const items = [
  ...
  { path: '/MainDB', icon: History, label: { en: 'History', fr: 'Historique' } },
  { path: '/MapsCoverage', icon: BarChart3, label: { en: 'Maps', fr: 'Cartes' } },
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 h-16 bg-card border-t border-border flex items-center justify-around lg:hidden">
      {items.map(item => {
        ...
        const isHistorical = ['/MainDB', '/MapsCoverage'].includes(item.path);
        return (
          <Link
            key={item.path}
            to={item.path}

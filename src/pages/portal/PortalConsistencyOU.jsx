...
export default function PortalConsistencyOU() {
  ...
  const columns = [
    ...
    {
      key: 'total_doses', header: t('Total doses','Total Doses'),
      render: (rowEntries) => {
        const total = ['Penta1','Penta3','MCV1','MCV2','OPV1','OPV3'].reduce((s, a) => s + sumDoses(rowEntries, a), 0);
        return <span className="text-xs">{total.toLocaleString()}</span>;
      },
    },
  ];
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        <h1 className="text-lg font-bold">{t('Cohérence par unité opérationnelle','Consistency by Operational Unit')}</h1>
        <p className="text-xs text-muted-foreground">{t('Taux d\'abandon par aire de santé','Dropout rates per health area')}</p>
      </div>
      {isLoading ? (
